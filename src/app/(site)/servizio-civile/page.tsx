...
import { SERVIZIO_CIVILE_QUERY, SETTINGS_QUERY } from "@/sanity/lib/queries";
import type { ServizioCivile, Settings } from "@/lib/types";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildMetadata(sc?.seo, {
    ...
    description:
      "Vivi un anno dalla parte giusta con la Misericordia di Gello: scopri i progetti di Servizio Civile Universale e Regionale.",
  });
}

export default async function ServizioCivilePage() {
  const [sc, settings] = await Promise.all([
    client.fetch<ServizioCivile | null>(
      SERVIZIO_CIVILE_QUERY,
      {},
      { next: { tags: ["servizioCivile"] } },
    ),
  ...
  return (
    <main>
      ...
      {sc?.tipi && sc.tipi.length > 0 && (
        <ScApplySection tipi={sc.tipi} emailSC={settings?.emailSC} />
      )}
    </main>
  );
}
